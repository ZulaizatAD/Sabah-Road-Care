...
class DuplicationService:
    ...
    # ----------------------
    @staticmethod
    def get_user_recent_reports(db: Session, user_id: int, hours_back: int = DEFAULT_HOURS_BACK) -> List[PotholeReport]:
    ...
    def get_nearby_reports(
        ...
        # Fetch and refine with precise distance
        reports = query.all()
        return [
            r for r in reports
            if calculate_distance(latitude, longitude, r.latitude, r.longitude) <= radius_meters
        ]
    ...
    def analyze_user_duplicates(
        ...
        radius_meters: int = DEFAULT_RADIUS_METERS,
        blocking_hours: int = DEFAULT_HOURS_BACK
    ) -> Dict[str, Any]:
    ...
    def calculate_priority_and_severity(similar_count: int, unique_users: int, base_severity: str = "Low") -> Dict[str, Any]:
        ...
        boosted_level = min(3, math.ceil(base_level * severity_multiplier))  # 🔥 ceil avoids downgrading
    ...
    def check_duplicate_submission(
        ...
        user_recent_reports = DuplicationService.get_user_recent_reports(db=db, user_id=user_id)
    ...
    def _generate_summary_message(user_analysis: Dict, similar_analysis: Dict, priority_analysis: Dict) -> str:
        ...
        if not user_analysis['can_submit']:
            count = len(user_analysis['duplicates'])
            if count == 1:
                d = user_analysis['duplicates'][0]
                return (
                    f"Cannot submit: You already reported this location "
                    f"{d['hours_ago']:.1f} hours ago. "
                    f"Please wait {d['remaining_hours']:.1f} more hours."
                )
            return f"Cannot submit: You already have {count} reports for this area within the last {user_analysis['blocking_hours']} hours."
...
def check_duplicate_reports(db: Session, user_id: int, latitude: float, longitude: float, radius_meters: int = DEFAULT_RADIUS_METERS):
